...
export default function MapLocation() {
  ...
  const mapsQuery = config.google_maps_url
    ? config.nome
    : `${config.nome} ${config.cidade || ''} ${config.uf || ''}`
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        <div className="text-center mb-12 scroll-reveal">
          ...
          <p className="text-gray-500 mt-4 max-w-xl mx-auto">
            Venha nos visitar{config.cidade ? ` em ${config.cidade}` : ''}. Estamos prontos para atender você e o seu negócio da beleza.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-8 items-stretch scroll-reveal">
          <div className="relative">
            ...
            <div className="map-float absolute -top-4 -right-4 hidden sm:flex items-center gap-3 bg-white rounded-2xl shadow-xl px-4 py-3 border border-gray-100">
              ...
              <div>
                <p className="text-xs font-bold text-text leading-tight">{config.nome}</p>
                <p className="text-[11px] text-gray-400 leading-tight">{config.cidade || ''}{config.uf ? ` · ${config.uf}` : ''}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow-lg flex flex-col justify-center border border-gray-100">
            ...
            <h3 className="font-bold text-xl mb-6">{config.nome}</h3>
            <ul className="space-y-4 mb-8">
              <li className="flex items-start gap-3">
                <span className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                  <MapPin className="w-5 h-5 text-primary" />
                </span>
                <span className="text-gray-600 text-sm mt-1.5">{config.endereco || config.cidade || ''}</span>
              </li>
              <li className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                  <Phone className="w-5 h-5 text-primary" />
                </span>
                <a href={`tel:${config.phone || config.whatsapp}`} className="text-gray-600 text-sm hover:text-primary transition-colors">
              ...
              <li className="flex items-start gap-3">
                ...
                <span className="text-gray-600 text-sm mt-1.5">{config.horario || ''}</span>
              </li>
              <li className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                  <Mail className="w-5 h-5 text-primary" />
                </span>
                <span className="text-gray-600 text-sm break-all">{config.email || 'Disponível pelo formulário do site'}</span>
              </li>
            </ul>
            ...
            >
              <Navigation className="w-4 h-4" /> Traçar rota até o {config.nome}
            </a>
          </div>
        </div>
      </div>
